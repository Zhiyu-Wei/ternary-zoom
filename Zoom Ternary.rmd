...
In this example, we aim to visualize how a three-part composition—Sleep, Exercise, and Others—relates to a physiological outcome using a custom ternary plot.

The dataset contains both compositional variables (proportions that sum to 1) and non-compositional variables (e.g., age, gender, physiological measurements). We also include a target variable (e.g., heart rate) as the response to be visualized in color.

This dataset is from Kaggle: https://www.kaggle.com/datasets/uom190346a/sleep-health-and-lifestyle-dataset
...
The compositional part (data2) must be explicitly converted into a matrix, as required by the plotting function.
...
#先找出data2(比例資料) & data1(非比例資料) & obj (應變數)
obj=finaldata[,7]
data1=finaldata[,c(1,2,4:6,8:11)]
data2=as.matrix(finaldata[,c(12:14)]) #記得要轉成matrix
```


use ggtern package to plot the ternary plot, we can see most of the data points are concentrated in the lower right corner.
...
Remember to detach the ggtern package after use to avoid conflicts with ggplot2 packages.
